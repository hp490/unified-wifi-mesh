...
int em_steering_t::send_client_assoc_ctrl_req_msg()
{
    ...
    if (assoc_param->sta_count == 0 || assoc_param->sta_count > MAX_STA_LIST) {
        em_printfout("%s:%d ERROR: Invalid sta_count=%u (max %u)", __func__, __LINE__, assoc_param->sta_count, MAX_STA_LIST);
        set_state(em_state_ctrl_configured);
        return -1;
    }

    if (assoc_param->assoc_control != 0x00 && assoc_param->assoc_control != 0x01) {
        em_printfout("%s:%d ERROR: Invalid assoc_control=%u. Allowed: 0 (Block), 1 (Unblock)",
                 __func__, __LINE__, assoc_param->assoc_control);
        set_state(em_state_ctrl_configured);
...
int em_steering_t::handle_client_assoc_ctrl_req(unsigned char *buff, unsigned int len)
{
    ...
    uint8_t sta_count = assoc_ctrl_req->sta_count;
    if (sta_count == 0 || sta_count > MAX_STA_LIST) {
        em_printfout("%s:%d: exceeding STA count=%u (max %u)", __func__, __LINE__, sta_count, MAX_STA_LIST);
        return -1;
    }

    // Collect indices of associated and unassociated STAs
    std::vector<uint8_t> associated_sta_indices;
    std::vector<uint8_t> unassociated_sta_indices;

    if (assoc_ctrl_req->assoc_control == 0x00) {
        ...
        for (uint8_t i = 0; i < sta_count; i++) {
            ...
            if (dm->is_sta_associated(assoc_ctrl_req->bssid, sta_mac)) {
                ...
                unassociated_sta_indices.push_back(i);
            }
        }
    ...
    } else if (assoc_ctrl_req->assoc_control == 0x01) {
        std::vector<const unsigned char*> list;
        for (uint8_t i = 0; i < sta_count; i++) {
