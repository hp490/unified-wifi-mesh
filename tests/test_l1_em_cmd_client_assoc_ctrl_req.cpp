...
TEST(em_cmd_client_assoc_ctrl_req_t, valid_standard_parameters)
{
    ...
    params.assoc_control = 1;  // block
    params.validity_period = 30;
    params.sta_count = 1;
    memcpy(params.sta_list[0], sta_mac, 6);

    EXPECT_NO_THROW({
        em_cmd_client_assoc_params_t obj(params);
        ...
        EXPECT_EQ(memcmp(obj.m_param.u.client_assoc_ctrl_req_params.bssid,
                         bssid, 6), 0);

        EXPECT_EQ(obj.m_param.u.client_assoc_ctrl_req_params.assoc_control, 1);
        EXPECT_EQ(obj.m_param.u.client_assoc_ctrl_req_params.validity_period, 30);
        EXPECT_EQ(obj.m_param.u.client_assoc_ctrl_req_params.sta_count, 1);

        EXPECT_EQ(memcmp(obj.m_param.u.client_assoc_ctrl_req_params.sta_list[0],
                         sta_mac, 6), 0);
 ...
 * -------------------------------------------------------------
 */
TEST(em_cmd_client_assoc_ctrl_req_t, valid_zero_parameters)
{
    std::cout << "Entering valid_zero_parameters test" << std::endl;

    em_cmd_client_assoc_params_t params{};
    memset(&params, 0, sizeof(params));

    EXPECT_NO_THROW({
        em_cmd_client_assoc_ctrl_req_t obj(params);

        EXPECT_EQ(obj.m_type, em_cmd_type_client_assoc_ctrl_req);
        EXPECT_STREQ(obj.m_name, "client_assoc");

        EXPECT_EQ(obj.m_param.u.client_assoc_ctrl_req_params.assoc_control, 0);
        EXPECT_EQ(obj.m_param.u.client_assoc_ctrl_req_params.validity_period, 0);
        EXPECT_EQ(obj.m_param.u.client_assoc_ctrl_req_params.sta_count, 0);
...
TEST(em_cmd_client_assoc_ctrl_req_t, valid_max_boundary_values)
{
    ...
    EXPECT_NO_THROW({
        ...
        EXPECT_EQ(memcmp(obj.m_param.u.client_assoc_ctrl_req_params.bssid, ff, 6), 0);
        EXPECT_EQ(obj.m_param.u.client_assoc_ctrl_req_params.validity_period, 65535);
        EXPECT_EQ(obj.m_param.u.client_assoc_ctrl_req_params.sta_count, 1);

        EXPECT_EQ(memcmp(obj.m_param.u.client_assoc_ctrl_req_params.sta_list[0], ff, 6), 0);
...
TEST(em_cmd_client_assoc_ctrl_req_t, multiple_sta_entries)
{
    ...
    EXPECT_NO_THROW({
        ...
        EXPECT_EQ(obj.m_param.u.client_assoc_ctrl_req_params.sta_count, 2);

        EXPECT_EQ(memcmp(obj.m_param.u.client_assoc_ctrl_req_params.sta_list[0], mac1, 6), 0);
        EXPECT_EQ(memcmp(obj.m_param.u.client_assoc_ctrl_req_params.sta_list[1], mac2, 6), 0);
